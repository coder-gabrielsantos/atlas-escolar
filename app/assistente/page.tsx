      ...
      <div className="mx-auto max-w-[1280px] px-5 py-7 sm:px-8 lg:px-10 lg:py-9">
        <section className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
          <div>
            ...
            <h1 className="mt-2 text-[clamp(2rem,4vw,3.35rem)] font-semibold leading-[1.02] tracking-[-0.052em]">
              Converse com o Atlas.
            </h1>
            <p className="mt-3 max-w-2xl text-sm leading-relaxed text-[var(--muted)] sm:text-base">
              Pergunte aos dados em linguagem natural e veja as evidências
              usadas na resposta.
            </p>
          </div>
          <Button
        ...
        <section className="mt-8 overflow-hidden rounded-xl border border-[var(--line)] bg-white shadow-[0_20px_70px_rgb(23_35_46/6%)]">
          <header className="flex flex-wrap items-center justify-between gap-4 border-b border-[var(--line)] bg-[#fbfbf8] px-5 py-4 sm:px-7">
            <div className="flex items-center gap-3">
              <div className="relative grid size-11 place-items-center rounded-lg bg-[var(--navy)] text-[var(--lime)]">
                <Bot size={20} />
                <span className="absolute -bottom-0.5 -right-0.5 size-3 rounded-full border-2 border-white bg-[#64b783]" />
              </div>
              <div>
          ...
          <div className="grid min-h-[610px] lg:grid-cols-[1fr_260px]">
            <div className="flex min-w-0 flex-col">
              <div className="soft-scroll max-h-[610px] flex-1 space-y-6 overflow-y-auto px-5 py-7 sm:px-8">
                {messages.map((message) => (
                  ...
                  >
                    ...
                    <div
                      className={`max-w-[86%] rounded-2xl px-4 py-3.5 ${message.role === 'user' ? 'bg-[var(--navy)] text-white' : 'border border-[var(--line)] bg-[#fbfbf8]'}`}
                    >
                      <RichText text={message.text} />
                      {message.chart === 'infrastructure' && (
                        <div className="chart-scroll mt-4 rounded-xl bg-white p-3">
                      ...
                      {message.role === 'assistant' && message.source && (
                        <div className="mt-4 border-t border-[var(--line)] pt-3 text-[10px] leading-5 text-[var(--muted)]">
              ...
              >
                <div className="flex items-end gap-3 rounded-2xl border border-[var(--line-strong)] bg-white p-2 shadow-sm focus-within:border-[var(--teal)]">
                  <Textarea
                    ...
                    placeholder="Pergunte sobre os dados desta escola…"
                  ...
                  <Button
                    ...
                    className="size-11 shrink-0 rounded-xl bg-[var(--teal)] text-white"
                  >
                    <ArrowUp size={18} />
                  </Button>
                </div>
                <p className="mx-auto mt-2 max-w-4xl text-center text-[10px] text-[var(--muted)]">
                  Dados agregados e institucionais. Confirme evidências com a
                  escola antes de decidir.
                </p>
              </form>
            </div>
              ...
              <div className="space-y-2">
                {SUGGESTIONS.map(({ label, question, icon: Icon }) => (
                  <button
                    ...
                    disabled={loading}
                    className="flex w-full items-center gap-3 rounded-xl border border-[var(--line)] bg-white p-3 text-left text-xs font-medium transition hover:border-[var(--teal)]"
                  >
                    <Icon size={15} className="shrink-0 text-[var(--teal)]" />
                    {label}
                  </button>
                ))}
              </div>
